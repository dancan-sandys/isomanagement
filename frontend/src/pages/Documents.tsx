...
const Documents: React.FC = () => {
  ...
  const roleCreate = hasRole(currentUser, 'QA Manager') || hasRole(currentUser, 'QA Specialist') || hasRole(currentUser, 'Production Manager') || isSystemAdministrator(currentUser);
  const roleApprove = hasRole(currentUser, 'QA Manager') || isSystemAdministrator(currentUser);
  const roleDelete = hasRole(currentUser, 'QA Manager') || isSystemAdministrator(currentUser);
  const roleUpdate = hasRole(currentUser, 'QA Manager') || hasRole(currentUser, 'QA Specialist') || hasRole(currentUser, 'Production Manager') || isSystemAdministrator(currentUser);

  const canCreateDocuments = hasDocPerm('create') || (currentUser?.permissions?.length === 0 && roleCreate);
  const canApproveDocuments = hasDocPerm('approve') || (currentUser?.permissions?.length === 0 && roleApprove);
  const canDeleteDocuments = hasDocPerm('delete') || (currentUser?.permissions?.length === 0 && roleDelete);
  const canUpdateDocuments = hasDocPerm('update') || (currentUser?.permissions?.length === 0 && roleUpdate);
  const canManageDocuments = canUpdateDocuments;
    ...
    <Box>
      ...
      {/* Document Table */}
        ...
        <Alert severity="info" sx={{ mb: 2 }}
          action={
            <Stack direction="row" spacing={1}>
              {canUpdateDocuments && <Button size="small" onClick={() => handleBulkAction('archive')} startIcon={<Archive />}>Archive</Button>}
              {canUpdateDocuments && <Button size="small" color="warning" onClick={() => handleBulkAction('obsolete')} startIcon={<Warning />}>Mark Obsolete</Button>}
