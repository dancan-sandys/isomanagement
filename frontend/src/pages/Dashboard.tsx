      ...
      <Grid item xs={12}>
        <Paper sx={{ p: 3 }}>
          ...
          <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap>
            {(hasPermission(user, 'users', 'read') || hasPermission(user, 'users', 'write')) && (
              <Button variant="contained" startIcon={<People />} onClick={() => window.location.href = '/users'}>
                Manage Users
              </Button>
            )}
            {(hasPermission(user, 'settings', 'view') || hasPermission(user, 'settings', 'update')) && (
              <Button variant="outlined" startIcon={<Settings />} onClick={() => window.location.href = '/settings'}>
                System Settings
              </Button>
            )}
            {(hasPermission(user, 'roles', 'read') || hasPermission(user, 'roles', 'write')) && (
