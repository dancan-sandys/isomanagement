...
import { canManageUsers, hasPermission } from '../store/slices/authSlice';
...
const Users: React.FC = () => {
  ...
  const hasUserManagementPermission = canReadUsers || canManageUsers(currentUser);
  ...
  const fetchRoles = async () => {
    try {
      const list = await rbacAPI.getRoles();
      const arr = Array.isArray(list) ? list : (list as any)?.data ?? (list as any)?.items ?? [];
      setRolesList(arr.map((r: any) => ({ id: r.id, name: r.name ?? 'Unnamed' })));
    } catch (err) {
  ...
  const handleCreateUser = async () => {
    ...
    const roleId = parseInt(userForm.role_id, 10);
    if (!Number.isInteger(roleId) || roleId < 1) {
      setFormErrors((e) => ({ ...e, role_id: 'Please select a valid role' }));
      return;
    }
    try {
      setLoading(true);
      ...
      if (userFormDepartmentId) {
        ...
        payload.department_id = parseInt(userFormDepartmentId, 10);
      }
      if (userForm.bio?.trim()) payload.bio = userForm.bio.trim();
  ...
  const getPasswordPolicyErrors = (password: string): string[] => {
    const errs: string[] = [];
    ...
    if (!/[^A-Za-z0-9]/.test(password)) errs.push('one special character (e.g. @ # $)');
    return errs;
  };
  ...
  const validateUserForm = (isEdit: boolean): boolean => {
    ...
    if (!isEdit) {
      const policyErrs = getPasswordPolicyErrors(userForm.password);
      if (policyErrs.length > 0) errors.password = `Password must include: ${policyErrs.join(', ')}`;
      ...
      {activeTab === 1 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            ...
            {(canWriteUsers || (!currentUser?.permissions?.length && canManageUsers(currentUser))) && (
              ...
              <TableBody>
                {users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      <IconButton size="small" onClick={() => { setSelectedUser(user); setViewUserDialogOpen(true); }} title="View"><VisibilityIcon /></IconButton>
                      {false && (
                      ...
                      {(canWriteUsers || (!currentUser?.permissions?.length && canManageUsers(currentUser))) && (
                        <>
                          ...
                          {user.is_active ? (
                            <IconButton size="small" onClick={() => handleDeactivateUser(user.id)} title="Deactivate"><LockIcon /></IconButton>
                          ) : (
                            <IconButton size="small" onClick={() => handleActivateUser(user.id)} title="Activate"><LockOpenIcon /></IconButton>
                          )}
                        </>
                      )}
                      {(canDeleteUsers || (!currentUser?.permissions?.length && canManageUsers(currentUser))) && (
                        <IconButton size="small" onClick={() => handleDeleteUser(user.id)} title="Delete"><DeleteIcon /></IconButton>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
