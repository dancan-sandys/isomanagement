  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchRoles.fulfilled, (state, action) => {
        ...
        state.roles = list.filter((r: Role) => r != null && r.id != null && typeof r.name === 'string');
      })
    ...
    builder
      ...
      .addCase(createRole.fulfilled, (state, action) => {
        state.loading = false;
        const role = action.payload?.data ?? action.payload;
        if (role && role.id != null && typeof role.name === 'string') {
          state.roles.push(role as Role);
        }
      })
