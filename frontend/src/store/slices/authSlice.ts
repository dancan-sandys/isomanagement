...
/** Check if user has a specific permission (module:action). Use for gating Create/Edit/Delete/etc. buttons. */
export const hasPermission = (user: User | null, module: string, action: string): boolean => {
  ...
  return user.permissions.some((p: string) => (p || '').toLowerCase() === key);
};
