...
def _user_permission_strings(db: Session, user_id: int) -> list:
    """Return list of permission strings for the user, e.g. ['haccp:view', 'documents:create']."""
    rbac = RBACService(db)
    perms = rbac.get_user_permissions(user_id)
    return [f"{getattr(p.module, 'value', p.module)}:{getattr(p.action, 'value', p.action)}" for p in perms]
...
async def login(
    ...
    permission_strings = _user_permission_strings(db, user.id)
    # Create user response with role name and permissions
...
async def get_current_user_info(
    ...
    permission_strings = _user_permission_strings(db, current_user.id)
    # Create user response with role name and permissions
    user_response = UserResponse(
        id=current_user.id,
