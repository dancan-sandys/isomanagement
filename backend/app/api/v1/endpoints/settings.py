...
from app.core.config import Settings as AppConfig
...
async def get_system_info(
    ...
    try:
        ...
        return ResponseModel(success=True, message="System info", data=data)
...
async def get_backup_status(
    ...
    """
    Return backup status. Placeholder implementation: no scheduler wired yet.
    """
    try:
        ...
        data = {
            ...
            "details": "No backup scheduler configured" if not last_backup else "Last backup completed",
        }
        return ResponseModel(success=True, message="Backup status", data=data)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fetch backup status: {str(e)}")
