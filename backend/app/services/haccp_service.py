...
class HACCPService:
    ...
    def create_verification_log_with_role_check(self, ccp_id: int, log_data: dict, created_by: int, allow_override: bool = False) -> CCPVerificationLog:
        ...
        # Store batch/value/unit in test_results (same shape as monitoring log) when provided
        ...
        # Determine verification status from measured value vs CCP limits (same logic as monitoring)
        measured_value = log_data.get("measured_value")
        verification_result = log_data.get("verification_result")
        is_compliant = log_data.get("is_compliant", True)
        if measured_value is not None and not isinstance(measured_value, (int, float)):
            try:
                measured_value = float(measured_value)
            except (TypeError, ValueError):
                measured_value = None
        if measured_value is not None and ccp is not None:
            is_compliant = True
            if ccp.critical_limit_min is not None and measured_value < ccp.critical_limit_min:
                is_compliant = False
            if ccp.critical_limit_max is not None and measured_value > ccp.critical_limit_max:
                is_compliant = False
            verification_result = "in_spec" if is_compliant else "out_of_spec"
