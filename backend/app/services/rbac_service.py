...
class RBACService:
    ...
    def get_role_summary(self) -> List[Dict[str, Any]]:
        ...
        for role in roles:
            ...
            # Convert permissions to proper format (module/action are stored as strings in DB)
            permissions_data = []
            for perm in role.permissions:
                module_str = getattr(perm.module, "value", perm.module)
                action_str = getattr(perm.action, "value", perm.action)
    ...
    def get_permission_matrix(self) -> Dict[str, Any]:
        ...
        for role in roles:
            ...
            for module in modules:
                module_str = getattr(module, "value", module)
                module_perms = []
                for permission in role.permissions:
                    perm_module = getattr(permission.module, "value", permission.module)
                    if perm_module == module_str:
                        action_str = getattr(permission.action, "value", permission.action)
                        module_perms.append(action_str)
                role_perms[module_str] = module_perms
