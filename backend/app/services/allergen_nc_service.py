...
class AllergenNCService:
    ...
    def _generate_nc_description(
        ...
        description_parts = [
            ...
            f"Detection Method: {flag.metadata.get('detection_method', 'Unknown') if flag.metadata else 'Unknown'}",
            f"Confidence Score: {flag.metadata.get('confidence_score', 'N/A') if flag.metadata else 'N/A'}",
            f"",
            f"ISSUE DESCRIPTION:",
            f"{flag.description}",
            f"",
            f"DETECTION DETAILS:",
            f"- Source: {flag.metadata.get('source', 'Not specified') if flag.metadata else 'Not specified'}",
            f"- Match Term: {flag.metadata.get('match_term', 'Not specified') if flag.metadata else 'Not specified'}",
            f"",
    ...
    def _create_evidence_files_list(
        ...
        evidence = {
            ...
            "detection_data": {
                ...
                "confidence": flag.metadata.get("confidence_score") if flag.metadata else None,
                "detection_method": flag.metadata.get("detection_method") if flag.metadata else None
            },
            "screenshots": [],
            "test_results": [],
